...
export class ConversionPipeline {
  ...
  async getCurrentPackageManager(): Promise<"npm" | "yarn" | "pnpm" | "bun"> {
    if (fs.existsSync(path.join(this.options.projectPath, "bun.lock"))) {
      return "bun"
    }
    if (fs.existsSync(path.join(this.options.projectPath, "pnpm-lock.yaml"))) {
      return "pnpm"
    }
    if (fs.existsSync(path.join(this.options.projectPath, "yarn.lock"))) {
      return "yarn"
    }
    if (
      fs.existsSync(path.join(this.options.projectPath, "package-lock.json"))
    ) {
      return "npm"
    }
  ...
  private async verifyNextJsProject() {
    ...
    if (this.options.packageJsonPath) {
      this.log(
        `Using custom package.json path: ${this.options.packageJsonPath}`
      )
    }
  ...
  private async installDependencies() {
    ...
    try {
      ...
      const hyphenSaveDev = ["npm"]

      const pm = await this.getCurrentPackageManager()
      this.log(`Detected package manager: ${pm}`)
      const i = pm === "yarn" ? "add" : "install"
      const saveDev = hyphenSaveDev.includes(pm) ? "--save-dev" : "-D"
      await execAsync(
        `${pm} ${i} ${saveDev} @opennextjs/cloudflare@latest wrangler@latest`,
        {
          cwd: installDir,
        }
      )
...
`
  ...
  private async updateGitignore() {
    ...
    if (this.options.packageJsonPath && !fs.existsSync(gitignorePath)) {
      const subfolderGitignore = path.join(
        this.options.projectPath,
        this.options.packageJsonPath,
        ".gitignore"
      )
      if (fs.existsSync(subfolderGitignore)) {
        ...
        this.log(
          `Using .gitignore from subfolder: ${this.options.packageJsonPath}/.gitignore`
        )
      }
    }
